...
function PutOrderModal () {
    ...
    function findOrder() {
        fetch(baseUrl+'order/id/'+orderId).then((response) => {
            ...
            updateCustomerId(data.customer.id);
            updateShipmentId(data.shipment.id);
    ...
    function submit() {
        ...
        const order = {id: orderId, customer: customer, shipment: shipment, orderDate: date, products: products};
        const json = JSON.stringify(order);
        console.log(json);
        fetch(baseUrl +'order/'+orderId, {method: 'PUT', body: json,
